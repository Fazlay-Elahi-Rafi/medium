        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="text-2xl font-bold text-center text-gray-800 dark:text-gray-800 p-4">
                    <h1>Edit Post: <span class="font-medium">{{ $post->title }}</span></h1>
                </div>
            </div>
                ...
                <form action="{{ route('post.update', $post->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <x-text-input id="title" class="block mt-1 w-full" type="text" name="title"
                            :value="old('title', $post->title)" required autofocus autocomplete="username" />
                        ...
                        <label for="dropzone-file"
                            ...
                            @if ($post->image)
                                <img id="preview-image" src="{{ asset('storage/' . $post->image) }}"
                                    class="absolute inset-0 w-full h-full object-contain z-0" alt="Current Image">
                            @else
                                <!-- Default upload UI -->
